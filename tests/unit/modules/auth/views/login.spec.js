...
describe('Pruebas en el Login Component', () => {
	...
	test('credendiales incorrectas, dispara el swal',async () => {
	...
	test('debe de redirigir a la ruta no-entry', async () => {
		...
		await txtEmail.setValue('victor@correo.com')
		await txtPassword.setValue('123456')

		await wrapper.find('form').trigger('submit')

		expect(store.dispatch).toHaveBeenCalledWith("auth/signInUser", {"email": "victor@correo.com", "password": "123456"})
